...
int getMinMens(Container *root){
	...
	if(aux->msg_head->prox != NULL && aux->last_msg < min_mens && aux->msg_head->prox->order == aux->last_msg + 1){
...
void sendMessages(Container *root) {
    ...
    //verificação se o min_answ precisa ser calculado novamente
	...
	while (aux != NULL) {
	    ...
	    // ela é enviada.
	    if (to_send != NULL && to_send->order == aux->last_msg + 1 && min_answ == aux->last_msg) {
		...
		// já que a mensagem fora enviada, podemos liberar o espaço alocado
		free(to_send);
